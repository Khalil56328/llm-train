...
class TrainTask(Base):
    ...
    base_model_version = Column(String(36), comment="基座模型版本（model_versions.id，32位UUID）")
    ...
    dataset_version = Column(String(36), comment="数据集版本")
    framework = Column(String(50), comment="训练框架（页面选择，入库回显；命令生成时定死 swift）")
    val_dataset_id = Column(String(36), comment="验证数据集ID（页面选择，入库回显）")
    val_dataset_version = Column(String(36), comment="验证数据集版本")
    sft_model_id = Column(String(36), comment="SFT模型ID（对齐任务，页面选择，入库回显）")
    sft_model_version = Column(String(36), comment="SFT模型版本（model_versions.id，32位UUID）")
    teacher_model_id = Column(String(36), comment="教师模型ID（压缩任务，页面选择，入库回显）")
    teacher_model_version = Column(String(36), comment="教师模型版本（model_versions.id，32位UUID）")
    calib_dataset_id = Column(String(36), comment="校准数据集ID（压缩任务，页面选择，入库回显）")
    calib_dataset_version = Column(String(36), comment="校准数据集版本")
