<template>
  <div class="evaluation-dataset">
    <PageHeaderCard
      title="评测数据集"
      desc="面向模型评测场景，提供集中式数据管理服务，支持多版本管控、数据导入导出等功能，实现评测数据从创建、存储、版本迭代到公开的全生命周期标准化治理。"
    />
        ...
        <el-dropdown trigger="click">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              ...
              <el-dropdown-item @click="openEditDialog(row as Dataset)">修改</el-dropdown-item>
...
import { getDatasetList, getDatasetDetail, updateDataset, deleteDataset as deleteDatasetApi } from '@/api/dataset'
import type { Dataset } from '@/types'

const router = useRouter()

const categories = [
  { label: '平台数据集', value: '平台数据集' },
  { label: '自定义数据集', value: '自定义数据集' },
]
...
function goCreate() {
  router.push({ name: 'data-evaluation-create' })
}

function goFiles(row: Dataset) {
  router.push({ name: 'data-evaluation-files', params: { id: row.id } })
}
...
async function deleteDataset(row: Dataset) {
  try { await deleteDatasetAction(row) } catch { /* noop */ }
}
