<template>
  <div class="pretrain-list">
    <PageHeaderCard title="预训练" desc="通过无标注数据进行无监督预训练，强化或新增模型特定能力。" />
    <SearchFilter v-model:model-value="searchKeyword" @search="fetchData" @reset="handleReset" @create="goCreate">
      <template #filters>
        ...
        <el-select v-model="filterTaskType" placeholder="任务类型" clearable style="width: 180px" @change="fetchData">
...
const columns: ColumnConfig[] = [
  ...
  { prop: 'subType', label: '任务类型', width: 120 },
...
async function fetchData() {
  ...
  try {
    const res = await getTrainTaskList({
      ...
      taskType: 'pretrain',
      taskSubType: filterTaskType.value || undefined,
...
function goCreate() {
  router.push('/train/pretrain/create')
}

function goEdit(row: any) {
  router.push({ path: '/train/pretrain/create', query: { id: row.id } })
}

function goDetail(row: any) {
  router.push({ path: `/train/task/${row.id}`, query: { from: TrainTaskTypeMenuMap.pretrain.path } })
}
