...
### 5. 默认模型（部署时自动安装）与真实模型下载

**平台不再内置演示种子模型**（旧版占位路径会导致真实训练前置检查失败）。`init_env.sh` 已内置**默认小模型自动安装**：

- 默认下载 **Qwen/Qwen2.5-0.5B-Instruct**（约 1GB）到 `backend/workspace/models/`，后端首次启动时**自动录入模型库**（模型广场可见，可直接用于微调/推理演示）；
- 更换模型：修改 `backend/.env` 的 `SEED_MODEL_ID`（ModelScope 模型 ID，留空 = 不自动安装）后重跑 `bash deploy/notebook/init_env.sh`；
- 手动安装：`bash deploy/notebook/download_models.sh --model <模型ID>` 下载后重启后端，启动时自动扫描录入。

> 0.5B 小模型可在无 GPU 机器上用 CPU 跑通演示链路；真实训练/推理建议按需下载更大模型。

按需下载其他真实模型 / 数据集（`download_models.sh` 会把模型下载到 `backend/workspace/models/` 并打印绝对路径）：
...
#   自定义目录：--dir models/qwen/qwen2.5-7b-instruct
```

然后在平台「模型管理 / 数据集管理」编辑对应记录的 `storage_path` 为脚本打印的绝对路径。

### 6. 备份与恢复

```bash
# 备份数据库 + 训练产物（mysqldump + workspace/storage 打包到 backend/backups/）
bash deploy/notebook/backup.sh
...
- **Notebook 实例释放后数据全部丢失**（磁盘不保留），请务必：
  ...
  2. **数据库**：定期 `bash deploy/notebook/backup.sh`（或手动 `mysqldump -u llm_train -p llm_train > backup.sql`）并下载；
  3. **代码**：每次改动后重新 `deploy/notebook/package.ps1` 打包上传，或维护一个 ModelScope 数据集仓库做代码中转；实例重建后 `unzip` + `bash deploy/notebook/init_env.sh` 即可恢复运行环境。
- **Ubuntu 服务器**：磁盘持久，但建议同样定期 `bash deploy/notebook/backup.sh` 并异地备份（backups/ 目录）。
